...
{
	if (OnSplit.ExecuteIfBound(GetSplitAmount(), GridIndex)) RemoveFromParent();
}

void UInv_ItemPopUp::DropButtonClicked()
{
	if (OnDrop.ExecuteIfBound(GridIndex)) RemoveFromParent();
}

void UInv_ItemPopUp::ConsumeButtonClicked()
{
	if (OnConsume.ExecuteIfBound(GridIndex)) RemoveFromParent();
}
